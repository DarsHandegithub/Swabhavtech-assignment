...
public class TeacherDao {
	...
	public boolean isTeacherPresent(int teacherId) {
		try {
			PreparedStatement preparedStatement = connection.prepareStatement("SELECT 1 FROM teachers WHERE teacher_id = ?"); 
	...
	public boolean isTeacherPresent(String firstName, String lastName) {
		try {
			PreparedStatement preparedStatement = connection.prepareStatement("SELECT 1 FROM teachers WHERE first_name = ? and last_name = ? and isActive = true");
	...
	public void searchATeacher(String firstName, String lastName) {
	    try {
	        preparedStatement = connection.prepareStatement("SELECT * FROM teachers WHERE first_name = ? and last_name= ? and isActive = true");
	        preparedStatement.setString(1, firstName);
	        preparedStatement.setString(2, lastName);
	        resultSet = preparedStatement.executeQuery();
	        String format = "| %-5s | %-15s | %-15s | %-15s | %-25s |%n";
	...
	public List<TeacherProfile> getAllTeacherProfiles() {
	    ...
	    String sql = "select * from teacher_profile where teacher_id in (select teacher_id from teachers where isActive=true)";
